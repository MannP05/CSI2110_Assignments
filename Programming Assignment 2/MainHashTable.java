...
class HashTable implements IMemoizedStorage {
    // boolean is just a placeholder empty value, we only care about table presence
    private HashMap<Integer, HashMap<Integer, Boolean>> lookupTable;

    private int key2Initial;

    public HashTable(int key1Initial, int key2Initial) {
        this.lookupTable = new HashMap<>(key1Initial); 
        this.key2Initial = key2Initial;
    }

    public boolean hasKey(int key1, int key2) {
        HashMap<Integer, Boolean> table = this.lookupTable.get(key1);
    ...
    public void storeKey(int key1, int key2) {
        ...
        if (table == null) {
            table = new HashMap<>();
            table.put(key2, true);
            this.lookupTable.put(key1, table);
            return;
        }
        table.put(key2, true);
    }
...
class MainHashTable {
    ...
    public void process() throws Exception {
        ...
        if (scanner.hasNextInt()) {
            ...
            for (int i=0; i< numTests; i++) {
                ...
                memoImplmentation = new HashTable(integers.size() * 2, 32);
    ...
    public static void main(String[] args) throws Exception {
        MainHashTable inputProcessor = new MainHashTable();
